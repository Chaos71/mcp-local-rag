...
При загрузке файла CLI выводит статус обработки дубликатов:
```
...
1. При загрузке файла вычисляется SHA-256 хеш содержимого
2. Через `vectorStore.getDuplicatesByHash()` проверяется наличие дубликатов в БД
...
> **Примечание:** Для работы требуется миграция схемы БД (таблица `duplicates` и поле `status` в таблице `chunks`).
