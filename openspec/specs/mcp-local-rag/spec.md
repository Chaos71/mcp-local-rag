...
### 5. Автоматическая загрузка .env файла

При запуске (CLI или MCP сервер) автоматически ищется файл `.env` в текущей рабочей директории. Если файл существует, переменные загружаются в `process.env` с сохранением приоритета внешней среды.
...
**Поведение:**
- Если файл `.env` существует — переменные загружаются
- Если файл `.env` не существует — система продолжает работу без ошибок
- Если файл `.env` пуст — система завершает загрузку без изменений
- Внешние переменные (из shell, docker, CI/CD) всегда имеют приоритет над `.env`

**Предупреждения о конфигурации:**
- При запуске система проверяет установленные переменные из `.env.example`
- Выводит список недостающих переменных в stderr
- Отключается через `RAG_QUIET_CONFIG=1`

**Автоматическое добавление в .gitignore:**
- При первой загрузке `.env` система проверяет наличие `.env` в `.gitignore`
- Если отсутствует — добавляет автоматически с предупреждением

### 6. PostgreSQL как бэкенд векторной БД
...
### AC-006: Автоматическая загрузка .env
**Дано:** В текущей рабочей директории существует файл `.env`
**Когда:** Приложение запускается
**Тогда:** Система загружает переменные из `.env` в `process.env`

### AC-007: Приоритет внешней среды
**Дано:** Переменная установлена в `process.env` из внешней среды
**Когда:** Приложение запускается и в `.env` существует та же переменная
**Тогда:** Значение из внешней среды сохраняется, значение из `.env` игнорируется

### AC-008: Безопасная обработка отсутствия .env
**Дано:** В текущей рабочей директории нет файла `.env`
**Когда:** Приложение запускается
**Тогда:** Система продолжает работу без ошибок
...
### S-005: Автоматическая загрузка .env
1. Пользователь создаёт файл `.env` в текущей директории с переменными:
   ```bash
   BASE_DIR=./docs
   DB_PATH=./lancedb
   MODEL_NAME=Xenova/all-MiniLM-L6-v2
   ```
2. Запускает mcp-local-rag без дополнительных настроек
3. Система автоматически загружает переменные из `.env`
4. Если некоторые переменные уже установлены в shell — они сохраняют приоритет
